...
CREATE TABLE financial_closings (
    ...
    start_date DATE NOT NULL,                           -- Period start date for included services
    end_date DATE NOT NULL,                             -- Period end date for included services
    total_amount DECIMAL(10,2) NOT NULL DEFAULT 0.00,  -- Sum of all service orders in this batch
    status VARCHAR(20) NOT NULL DEFAULT 'OPEN',         -- 'OPEN', 'CLOSED', 'INVOICED'
    created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP,

    -- Constraint to enforce valid batch status
    CONSTRAINT chk_closing_status CHECK (status IN ('OPEN', 'CLOSED', 'INVOICED'))
);

-- 2. Junction Table linking Service Orders to Financial Closing Batches
CREATE TABLE financial_closing_items (
    ...
    PRIMARY KEY (financial_closing_id, service_order_id)
);
